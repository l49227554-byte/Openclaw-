...
- Opening a thread shows a message-shaped skeleton while history is unavailable, then fades in the first available messages. Recently viewed transcripts, progress cards, and model labels replace the previous conversation together immediately after the sidebar closes and stay usable during refresh. Model labels show a compact activity indicator while resolving and fall back to the model ID only after resolution finishes without a friendly name. Android’s **Remove animations** setting disables the shimmer and fade.
