...
- Opening a thread shows a message-shaped skeleton while history is unavailable, then fades in the first available messages. Cached transcripts, progress cards, and model labels replace the previous conversation together and stay usable during refresh. Friendly model names are cached once per Gateway and agent for reuse across sessions; Gateway restarts, metadata/config events, reconnects, and a 24-hour access TTL trigger background revalidation without clearing the cached label. Model labels show a compact activity indicator only when no friendly name is cached and fall back to the model ID only after resolution finishes without a name. Android’s **Remove animations** setting disables the shimmer and fade.
