...
After that admission, canonical main pushes and trusted PRs selecting the fixed Windows pair can move five measured check rows to hosted Ubuntu: dependencies, both core test-type stripes, extension package boundaries, and runtime topology architecture. The separate `hybrid_hosted_checks` decision requires fresh healthy [assignment evidence](/ci/runners#hybrid-hosted-assignment-guard) and enough room for every selected check within the same 45-row limit. Precise PRs without that measured latency floor retain their existing routes.

Main pushes then consider lint and central test types under `hybrid_hosted_main_checks`. These jobs use at most four observed CPUs and had at least five minutes of finish slack in every sampled main run. Their slower hosted execution is acceptable for main's thirty-minute objective; PRs retain Blacksmith for both jobs. This admission consumes only the remaining hosted capacity and fails closed with the five-check decision. It does not change either job's timeout.

Control UI E2E shards, QA Smoke, real-Gateway checks, Android, and artifact builds retain their existing routes. Test inventory and worker limits remain unchanged. Security waits for preflight's original budget decision and still runs after a preflight failure unless the workflow was canceled. Hosted assignment delay and ordinary cache setup can offset compute savings, so compare the first exact-head run's hosted/Blacksmith row counts and queue times before widening admission.
