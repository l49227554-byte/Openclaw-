...
A final message-tool reply on the canonical send path can hand delivery to the Gateway's durable queue when its account, target, thread, and admitted source turn match. A `delivery_queued` result means the original reply is retained for retry or reconciliation, not that it has been delivered; do not resend it. Progress updates, other destinations, and provider-native actions retain their live-run delivery rules. Unknown provider outcomes are not blindly replayed.
