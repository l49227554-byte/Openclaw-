...
- `plugins.entries.<id>.acp.allowDetachedSpawn`: explicitly allow this bundled or trusted official plugin to spawn one-shot ACP harness runs through `api.runtime.acp` from detached work (timers, background jobs) with no live Gateway request. Request-scoped spawns never need it, and normal `acp.*` policy plus subagent admission limits still apply. See [Background work](/plugins/sdk-runtime/background-work#api-runtime-acp).
