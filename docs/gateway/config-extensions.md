...
- `plugins.entries.<id>.subagent.allowRun`: consents to that exact plugin's manifest-declared `runtimeCapabilities: ["subagent.run"]` when an `auth: "plugin"` HTTP route delegates a subagent launch. Both declaration and consent are required for that otherwise-unprivileged route. Existing Gateway-authorized and background runtime callers keep their normal caller authority. This does not grant general Gateway or operator access.
