...
<AccordionGroup>
  <Accordion title="3. Legacy state migrations (disk layout)">
    ...
    Before schema migrations or debug capture writes, full Doctor checks existing supported databases for conflicting bare `global` or `unknown` aliases. Repair holds maintenance ownership and refreshes this check; diagnostic Doctor checks before offering an update. If a bare alias and its agent-qualified target refer to different conversations, Doctor preserves their histories and schema versions, stops repair, and names both keys. Lint, post-upgrade probes, and targeted SQLite commands keep their selected scope and do not start persistent debug capture. Legacy file imports retain their existing backup and migration boundary.

    Agent-qualified session keys and lineage remain stable when `session.scope` changes. Doctor scopes legacy bare aliases using their recorded owner; it does not reinterpret already-qualified keys from the current routing configuration. The separately authorized retired-agent migration above retains its recorded ownership rules. The SQL schema and legacy `session_key_contract` table remain unchanged. Older builds can still reject keys under that table's old main-key policy, so returning to an older build requires restoring the pre-update backup rather than assuming same-schema rollback compatibility.
