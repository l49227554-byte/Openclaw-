...
<AccordionGroup>
  <Accordion title="3. Legacy state migrations (disk layout)">
    ...
    SQLite databases and their WAL, shared-memory, and rollback-journal files are one migration unit. If any destination family member already exists, Doctor preserves the entire legacy family together in quarantine and lists its files in the repair receipt. It never attaches a legacy WAL to another database. A source family missing its database stays in place with a deferred warning. If a later move fails before the legacy directory is retired, Doctor restores the completed moves so the SDK can keep using the original store. A failed rollback blocks migration and names the preserved recovery paths.
