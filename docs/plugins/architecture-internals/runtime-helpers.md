...
- An `auth: "plugin"` HTTP route has no operator scopes. To delegate `run(...)` from that route, the plugin manifest must declare `contracts.runtimeCapabilities: ["subagent.run"]`, and the operator must consent with `plugins.entries.<id>.subagent.allowRun: true`. The grant is bound to the declaring plugin id and exposes only this host-owned run path. Existing Gateway-authorized and background callers retain their normal authority and compatibility. The delegated turn is a host-owned system action, including on Gateways with operator roles enabled; it does not impersonate a human profile or inherit a route caller's role. Normal agent-access, session-ownership, model, tool, and commit-time revocation checks still apply.
