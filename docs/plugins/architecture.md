...
A later reload can retry after pending cleanup completes successfully, without capturing code from the disposed instance again. A timed-out admitted call also keeps retry blocked until that call finishes. Rejected replacements and failed recovery registrations retain the same cleanup barrier. Recovery restores only registrations that were usable when that reload began; it does not silently substitute current package files for an already retired registration whose captured source has been released.
