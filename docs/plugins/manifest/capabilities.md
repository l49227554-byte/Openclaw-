...
| `runtimeCapabilities`            | `string[]` | Privileged host-runtime operations requested by this plugin. Currently only `subagent.run` is recognized.                            |
...
`contracts.runtimeCapabilities` accepts `"subagent.run"`. Declaration alone grants
nothing: the operator must also set
`plugins.entries.<id>.subagent.allowRun: true`. Core binds the resulting
entitlement to the exact plugin id and uses it only for
`api.runtime.subagent.run(...)`; it does not expose generic Gateway dispatch or
operator scopes.
