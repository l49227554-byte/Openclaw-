...
<AccordionGroup>
  <Accordion title="api.runtime.agent">
    ...
    **Session store helpers** are under `api.runtime.agent.session`:
    ...
    Adopting versus spawning: `createSessionEntry(...)` with `acpSessionBinding` adopts an existing external agent session into a plugin-owned persistent session that the plugin drives turn by turn through its own ACP-backed actions. To start a fresh, one-shot harness run that OpenClaw executes, tracks in the task registry, and lets the plugin wait on, observe, or cancel, use [`api.runtime.acp.spawn(...)`](/plugins/sdk-runtime/background-work#api-runtime-acp) instead; do not hand-create an ACP session entry and drive it through raw manager calls.
