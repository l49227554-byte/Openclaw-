...
custom message and returns a promise for its persisted `messageId` and canonical
`message`. File-backed
notes use the same canonical agent worker and writer queue. The embedded runner
awaits its failed-image note before publishing that stored message in live context or the
completed result. Input and target capture precede awaited work; transaction and
