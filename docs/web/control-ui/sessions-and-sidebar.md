...
**Unpin Home.** Open **Edit pinned items** from the Pages customize menu (or right-click the navigation area) and clear **Home**. In single-agent mode, the shortcut disappears and the same conversation becomes available in the session list, where it can be assigned to a custom group. Check **Home** again to restore the shortcut and hide the ordinary main-session row. Its transcript, group metadata, and message routing are unchanged. Home stays pinned by default, and **Reset pinned items** restores it. This choice uses the same preference persistence as other navigation pins. Team mode continues to represent Home with the agent header.
