    ...
    const finishPrimitive = (): void => {
      if (capture && primitiveNeedsCapture) {
        if (primitiveRaw === "true") {
          ...
          // Numeric timestamps stay numeric; anything else is left unparsed so
          // the caller's date-first parser sees the same mixed-input contract.
          if (/^-?\d{1,15}$/.test(primitiveRaw)) {
            capture[currentPrimitiveKey] = Number(primitiveRaw);
          }
        }
      }
      primitiveRaw = "";
