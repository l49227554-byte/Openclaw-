...
function isOptionalDiaryContextReadError(err: unknown): boolean {
  ...
  return code === "EACCES" || code === "EPERM" || isEmptyDreamsReadErrorCode(code, err);
}
...
export async function appendNarrativeEntry(params: {
  ...
  return await updateDreamsFile<string | undefined>({
    ...
    updater: async (existing, dreamsPath) => {
      ...
      // Published context entries are already clamp-normalized; clamping them
      // again is not idempotent in the truncation regime, so compare directly.
      if (
