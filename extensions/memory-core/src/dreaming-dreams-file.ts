...
type ManagedMarkdownUpdateParams = {
  ...
  creationMode?: number;
  /** Route the final write through writeDreams instead of replaceReport. */
  remoteWrite?: "report" | "dreams";
  /** Skip the update entirely when the target file does not exist. */
...
export function isEmptyDreamsReadErrorCode(code: string | undefined, err: unknown): boolean {
  if (
    code === "ENOENT" ||
    code === "ENOTDIR" ||
    code === "not-found" ||
...
function isEmptyDreamsReadError(err: unknown): boolean {
  return isEmptyDreamsReadErrorCode(extractErrorCode(err), err);
}
...
export async function updateManagedDreamingMarkdownFile(
  ...
  const files = params.workspaceDir
    ...
    : undefined;
  if (files) {
    // Registered workspaces route the size check, read, and final write through
    // the maintenance owner so remote notes update and access is revalidated
    // before publication instead of writing a Gateway-local twin.
    let size: number | undefined;
    try {
      const info = await files.stat(params.filePath, true);
      if (!info.isFile) {
        throw new Error(`Refusing to write non-file markdown file: ${params.filePath}`);
      }
      size = info.size;
    } catch (err) {
      if (extractErrorCode(err) !== "ENOENT") {
    ...
    if (size === undefined && params.skipWhenAbsent) {
      return false;
    }
    if (size !== undefined && size > MEMORY_DREAMING_MARKDOWN_MAX_BYTES) {
      throw new Error(
        `Refusing to buffer managed Markdown file beyond ${MEMORY_DREAMING_MARKDOWN_MAX_BYTES} bytes: ${params.filePath}`,
      );
    }
    const original =
      size === undefined ? "" : (await files.readFile(params.filePath)).toString("utf-8");
    const updated = replaceManagedMarkdownBlock({ original, ...params });
    const content = withTrailingNewline(updated);
    if (params.remoteWrite === "dreams") {
      await files.writeDreams(params.filePath, content);
    } else {
      await files.replaceReport(params.filePath, content);
    }
    return true;
  }
  // Probe readability first: an unreadable daily file must propagate its
  // failure instead of being treated as absent, and an absent target must not
  // create its parent directory when the update would be skipped anyway.
  let absent = false;
  try {
  ...
  if (absent && params.skipWhenAbsent) {
    return false;
  }
  ...
  if (!resolved && params.skipWhenAbsent) {
    return false;
  }
    ...
    async () =>
      await withDreamsFileLock(params.workspaceDir, async (dreamsPath) => {
        ...
        if (shouldWrite) {
          ...
          if (files) {
            await files.writeDreams(dreamsPath, normalized);
          } else {
            await fs.mkdir(path.dirname(dreamsPath), { recursive: true });
            await writeDreamsFileAtomic(dreamsPath, normalized, params.workspaceDir);
          }
        }
        return result;
      }),
