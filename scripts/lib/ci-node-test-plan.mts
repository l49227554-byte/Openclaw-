...
function estimateCompactStripeSeconds(
  ...
  if (group.timing_key?.endsWith(COMMANDS_PARALLEL_TIMING_SUFFIX)) {
...
function splitOversizedCompactGroup(
  ...
  const timingParent = parallelCommands ? compactGroupTimingKey(group) : group.shard_name;
  const splitTimingPrefixes = [timingParent, group.shard_name].map((key) => `${key}#selector-`);
