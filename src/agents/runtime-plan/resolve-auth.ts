...
import { createRuntimeProviderAuthLookup, getApiKeyForModelCore } from "../model-auth.js";
...
export async function resolvePreparedRuntimeModelAuth(
  ...
  if (candidates.length === 0) {
    ...
    // The prepared runtimeLookup scopes eligibility in core so a stored-key
    // provider (e.g. openai) does not pay for plugin discovery just to fail.
