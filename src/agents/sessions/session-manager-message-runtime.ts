...
export async function appendSessionTranscriptMessage(input: {
  ...
  cwd: string;
  assertCurrent: () => void;
}): Promise<Pick<TranscriptMessageAppendResult<CustomMessage>, "messageId" | "message">> {
  ...
  try {
    ...
    await worker.run(async (scope) => {
      const reply = await scope.execute({
        ...
        input: {
          scope: { ...input.target, storePath: execution.path, env: options.env },
          message,
          cwd: input.cwd,
        },
      });
  ...
  return { messageId: committed.messageId, message: committed.message };
}
