...
const base = { task: "Check the upload race", sourceReplicaId: "parent", targetReplicaId: "child" };
const verifier = {
...
describe("native dynamics spawn preparation", () => {
  ...
  it("binds resolved profile and host-owned lineage into reproducible task bytes", () => {
    ...
    expect(first.task).not.toBe(prepareDynamicsSpawn({ ...base, dynamics: { profile: "builder" } }).task);
    expect(first.task).not.toBe(prepareDynamicsSpawn({ ...base, targetReplicaId: "replacement", dynamics: { profile: "explorer" } }).task);
  });
  it.each([null, [], { profile: "constructor" }, { profile: "explorer", authority: "admin" }, { profile: "independent-verifier" }])(
    "rejects invalid or misleading configuration %j",
    (dynamics) => {
      expect(() => prepareDynamicsSpawn({ ...base, dynamics })).toThrow();
    },
  );
  it("bounds handoffs and snapshots their content before returning", () => {
    expect(() => prepareDynamicsSpawn({ ...base, dynamics: { profile: "builder", handoff: { summary: "x".repeat(4097) } } })).toThrow();
    expect(() => prepareDynamicsSpawn({ ...base, dynamics: { profile: "critic", handoff: { evidenceRefs: Array(33).fill("ref") } } })).toThrow();
