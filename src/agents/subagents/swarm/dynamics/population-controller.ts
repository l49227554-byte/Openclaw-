...
export function buildPopulationSnapshot(params: {
  ...
  if (correlation !== null && (!Number.isFinite(correlation) || correlation < 0 || correlation > 1)) {
  ...
  for (const replica of params.replicas) {
    if (!replica.replicaId.trim() || replicaIds.has(replica.replicaId) ||
        replica.campaignId !== params.campaignId || replica.groupId !== params.groupId) {
      throw new Error("population replicas must have unique identities in the same campaign and group");
    }
  ...
  for (const assessment of assessments) {
    if (observedIds.has(assessment.replicaId) ||
...
export function assessPopulation(input: PopulationSnapshot): PopulationDecision {
  ...
  if (systemPressure >= 0.8 || mix.jammed >= 0.25) {
    actions.push({ kind: "drain", reason: "resource/context/debt pressure is too high for further expansion" });
    rationale.push("jammed pressure takes precedence over exploratory expansion");
    return { authority: "search-only", actions, rationale };
  }
  if (mix.critical >= 0.2) {
    const targets = snapshot.observations.filter((item) => assessLocalPhase(item).phase === "critical").map((item) => item.replicaId);
    actions.push({ kind: "measure", targetReplicaIds: targets, reason: "critical replicas require discriminating measurements before widening search" });
    rationale.push("critical behavior shifts budget from blind expansion to measurement");
  }
  if (mix.glass >= 0.15) {
    actions.push({ kind: "perturb", profile: "glass-breaker", count: 1, reason: "bounded fresh-context perturbation for stalled low-mobility search" });
    rationale.push("glassy lanes get one bounded decorrelated perturbation");
  }
  const frozenTargets = snapshot.observations.filter((item) => assessLocalPhase(item).phase === "crystal").map((item) => item.replicaId);
  if (frozenTargets.length > 0) {
    // Freeze eligible local lanes; unrelated hot exploration cannot dilute their evidence.
    actions.push({ kind: "freeze", targetReplicaIds: frozenTargets, reason: "low-entropy candidates with substantial evidence should be frozen for verification" });
  ...
  if (mix.gas >= 0.4 && mix.critical === 0 && systemPressure < 0.6) {
    actions.push({ kind: "spawn", profile: "builder", count: 1, reason: "broad exploration has enough diversity to justify one coordinating builder" });
