...
describe("managed worktree sandbox Git executor", () => {
  ...
  it("retries one transient fetch within the original deadline and rechecks authority", async () => {
    const root = await createRepositoryFixture();
    let fetchAttempts = 0;
    const beforeRun = vi.fn();
    const restore = registerSandboxBackend("docker", async () => ({
      id: "docker",
      runtimeId: "transient-fetch-test",
      runtimeLabel: "transient-fetch-test",
      workdir: "/workspace",
      buildExecSpec: vi.fn(),
      async runShellCommand(command) {
        command.beforeRun?.();
        ...
        if (command.args?.includes("fetch")) {
          ...
          return fetchAttempts === 1
            ? {
                ...
                stderr: Buffer.from("connection reset by peer\n"),
                code: 1,
              }
    ...
    const executor = await createWorktreeGitExecutor({
      isolation: {
        ...
        sessionKey: "agent:main:subagent:transient-fetch",
      },
      repoRoot: root,
      env: { OPENCLAW_STATE_DIR: path.join(root, "state") },
    });

    const result = await executor?.run(root, ["fetch", "origin"], {
      timeoutMs: 5_000,
      beforeRun,
    });
