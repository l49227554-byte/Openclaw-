...
import {
  ...
  withGitNetworkRetry,
} from "../../infra/git-network-retry.js";
...
async function runSandboxGit(params: {
  ...
  let latest: { stdout: Buffer; stderr: Buffer } = {
    stdout: Buffer.alloc(0),
    stderr: Buffer.alloc(0),
  };
  const result = await withGitNetworkRetry(
    retryableGitNetworkOperation(params.args),
    ...
    async (attemptTimeoutMs) => {
      ...
      try {
        ...
        return {
          ...
          signal: null,
          killed: true,
          termination: timedOut ? ("timeout" as const) : ("signal" as const),
      ...
      latest = { stdout: stdout.buffer, stderr: stderr.buffer };
      return {
