  ...
  async remove(params: RemoveWorktreeParams): Promise<RemoveManagedWorktreeResult> {
    ...
    try {
      const result = await this.withAllocationLease(params, async (guard) => {
        timing?.markPhase();
        try {
          // The caller guard owns the first admitted phase even when it rejects
          // before registry lookup or sandbox construction.
  ...
  async gc(params: ManagedWorktreeGcParams = {}): Promise<ManagedWorktreeGcResult> {
    ...
    for (const record of listRegistryWorktrees(this.env)) {
      ...
      try {
        ...
        if (!this.isolationForRecord(current) && !emptySource) {
          // Trusted repositories preserve the established non-blocking retention path.
          // Empty-workspace sources can be deleted during pruning and therefore
          // remain serialized with restore even when they do not require sandbox Git.
          await pruneSnapshot({});
          continue;
        }
        await this.withAllocationLease({}, async (guard) => {
          const admitted = getRegistryWorktree(this.env, record.id) ?? current;
          if (!(await worktreePathExists(admitted.repoRoot))) {
            await pruneSnapshot(guard);
            return;
          }
          await this.withRepositoryGit(admitted, () => pruneSnapshot(guard));
        });
      } catch (error) {
