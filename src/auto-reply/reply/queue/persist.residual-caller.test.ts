...
describe("durable follow-up residual production callers", () => {
  ...
  ])("recovers %s work, the residual population this queue exists for", (sourceTool, label) => {
    const queue = getFollowupQueue(TEST_KEY, SETTINGS);
    queue.items.push(systemProvenanceRun(`${label} turn`, sourceTool));
    persistFollowupQueuesOrThrow();

    expect(followupQueueEntryContainsPrompt(TEST_KEY, `${label} turn`)).toBe(true);
  ...
  it("leaves an operator chat.send to its pending-input receipt owner", () => {
    const queue = getFollowupQueue(TEST_KEY, SETTINGS);
    queue.items.push(
    ...
    expect(followupQueueEntryContainsPrompt(TEST_KEY, "operator turn")).toBe(false);
    expect(followupQueueEntryContainsPrompt(TEST_KEY, "system turn")).toBe(true);
