...
describe("repairMissingConfiguredPluginInstalls", () => {
  ...
  it("refreshes a stale ClawHub Codex runtime using its declared official catalog source", async () => {
    ...
    mocks.loadPluginMetadataSnapshot.mockReturnValue({
      plugins: [{ id: "codex", packageVersion: "2026.9.3", providers: ["codex"] }],
      diagnostics: [],
    });
    mocks.installPluginFromClawHub.mockResolvedValueOnce({
      ok: true,
    ...
    await mocks.listOfficialExternalPluginCatalogEntries.withImplementation(
      () => [
        {
          id: actualCatalog.resolveOfficialExternalPluginId(catalogEntry),
          label: actualCatalog.resolveOfficialExternalPluginLabel(catalogEntry),
          install: actualCatalog.resolveOfficialExternalPluginInstall(catalogEntry),
        },
      ],
      async () => {
        const result = await repairConfiguredPlugins(
