...
export async function recoverConfigFromJsonRootSuffixWithContext(
  ...
  // cwd (accepted before the strict check). Migrate saved blanks so recovery
  // persists the recoverable configuration; new authoring still fails strict
  // validation elsewhere.
  const migrated = migrateBlankAgentCwd(resolution.resolvedConfigRaw as OpenClawConfig);
