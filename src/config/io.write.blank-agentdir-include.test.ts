...
describe("saved blank agentDir from an include does not block unrelated writes", () => {
  it("an unrelated gateway.port write persists when the blank agentDir lives in an included file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "proof-151013-inc-"));
    fs.writeFileSync(path.join(root, "entries.json"), JSON.stringify({ alpha: { agentDir: " " } }));
    fs.writeFileSync(
    ...
    expect(persisted.agents.entries.$include).toContain("entries.json");
  });
});
