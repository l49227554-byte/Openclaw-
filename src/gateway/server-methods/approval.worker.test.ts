...
function createPendingFixture() {
  ...
  let stored: OperatorApprovalRecord | undefined;
...
it.each(["approval.get", "approval.resolve", "approval.history"] as const)(
  "rejects pre-revoked request custody before %s enters storage",
  async (method) => {
    const { managers, handlers, record, stored, deny, decision } = createPendingFixture();
    const lookup = vi
      .spyOn(operatorApprovalStore, "getOperatorApprovalDetailedAsync")
      .mockResolvedValue({ outcome: "found", record: stored });
    const history = vi
      .spyOn(operatorApprovalStore, "listTerminalOperatorApprovalsAsync")
      .mockResolvedValue({ records: [] });
    const nativeLookup = vi
      .spyOn(nativeOperatorApprovalStore, "getOperatorApprovalDetailed")
      .mockReturnValue({ outcome: "found", record: stored });
    const resolve = vi
      .spyOn(managers.exec, "resolveDetailed")
      .mockReturnValue({ outcome: "not-found" });
    let decided = false;
    void decision.then(() => {
      decided = true;
