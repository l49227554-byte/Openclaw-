  ...
  it("projects pending input acceptance times with fresh page-scoped sender displays", async () => {
    await withOpenClawTestState({ scenario: "minimal" }, async () => {
      ...
      try {
        ...
        const initialAvatar = asOptionalRecord(initialMessage?.__openclaw)?.senderProfileAvatarUrl;
        const updatedAvatar = asOptionalRecord(updatedMessage?.__openclaw)?.senderProfileAvatarUrl;
