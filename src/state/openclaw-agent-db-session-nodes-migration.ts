...
/** The same ordered projections feed migration writes and read-only collision preflight. */
export function createLegacySessionNodeSelects(
  db: DatabaseSync,
  ...
  if (readSqliteTableColumns(db, "sessions")) {
    // Retained generations need a node even without a live entry or route.
    selects.push({
      replace: false,
      columns: `session_key, current_session_id, entry_json, updated_at`,
      sql: `SELECT session_key, session_id, '{}', updated_at
...
export function createLegacySessionWindowSelect(
  db: DatabaseSync,
