...
import type { SessionInfo, TuiOptions, TuiSessionIntent } from "./tui-types.js";
...
export function createCommandHarness(params?: {
  ...
  const runAuthFlow: RunAuthFlow | undefined =
    params?.runAuthFlow ??
    (params?.opts?.local
      ? vi.fn<RunAuthFlow>().mockResolvedValue({
          exitCode: 0,
          signal: null,
  ...
  const state = {
    ...
    currentSessionIntent: params?.currentSessionIntent,
