...
import type { TuiBackend, TuiSessionDescription } from "./tui-backend.js";
...
describe("tui session actions", () => {
  ...
  it("queues session refreshes and applies the latest result", async () => {
    const firstDescription = createDeferred<TuiSessionDescription>();
    const secondDescription = createDeferred<TuiSessionDescription>();

    const describeSession = vi
      .fn<TuiBackend["describeSession"]>()
      .mockImplementationOnce(() => firstDescription.promise)
      .mockImplementationOnce(() => secondDescription.promise);
  ...
  it("skips UI work when session refresh metadata is unchanged", async () => {
    const describeSession = vi.fn<TuiBackend["describeSession"]>().mockResolvedValue({
      defaults: {},
      session: {
        key: "agent:main:main",
        model: "sonnet-4.6",
  ...
  it("describes the exact global session when refreshing its metadata", async () => {
    const describeSession = vi.fn<TuiBackend["describeSession"]>().mockResolvedValue({
      defaults: {},
      session: { key: "agent:main:global", updatedAt: 1 },
    });
    const state = createBaseState({
      currentSessionKey: "agent:main:global",
      sessionScope: "global",
    });
  ...
  it("keeps global session info aligned with selected-agent chat history", async () => {
    const describeSession = vi.fn<TuiBackend["describeSession"]>().mockResolvedValue({
      defaults: {},
      session: { key: "agent:work:global", updatedAt: 1 },
    });
    const state = createBaseState({
      currentAgentId: "work",
      currentSessionKey: "agent:work:global",
      sessionScope: "global",
    });
  ...
  it("ignores stale session info after switching away and back to the same key", async () => {
    ...
    const describeSession = vi.fn<TuiBackend["describeSession"]>(() => firstSessionInfoA.promise);
    const state = createBaseState({ currentSessionKey: "agent:main:home" });
  ...
  it.each(["none", "before adoption", "during refresh", "after acknowledgement"])(
    ...
    async (notification) => {
      ...
      const client = makeTuiBackend({
        ...
        describeSession: vi.fn<TuiBackend["describeSession"]>(() => {
  ...
  it("fences pre-reset history and session-info reads when reset commits", async () => {
    ...
    const loadHistory = vi.fn(() => history.promise);
    const describeSession = vi.fn<TuiBackend["describeSession"]>(() => sessionInfo.promise);
  ...
  it.each(["success", "failure"] as const)(
    ...
    async (outcome) => {
      ...
      const { refreshSessionInfo } = createTestSessionActions({
        client: makeTuiBackend({
          describeSession: vi.fn<TuiBackend["describeSession"]>(() => sessionInfo.promise),
        }),
        chatLog: makeChatLog({ addSystem }),
        state,
      });
  ...
  it.each([
    ...
    const { abortActive, setSession } = createTestSessionActions({
      client: makeTuiBackend({ loadHistory, abortChat }),
  ...
  it("remembers the selected session after history loads", async () => {
    ...
    expect(state.currentSessionKey).toBe("agent:main:main");
    expect(rememberSessionKey).toHaveBeenCalledWith(
  ...
  it("hydrates session info from chat history without another metadata read", async () => {
    const describeSession = vi.fn<TuiBackend["describeSession"]>();
    const loadHistory = vi.fn().mockResolvedValue({
      messages: [],
      sessionInfo: {
        key: "agent:main:main",
  ...
  it("loads selected-agent global history with the selected agent id", async () => {
    ...
    const describeSession = vi.fn<TuiBackend["describeSession"]>().mockResolvedValue({
      session: { key: "agent:work:global", sessionId: "session-work-global" },
    });
    const state = createBaseState({
      currentAgentId: "work",
      currentSessionKey: "global",
    ...
    expect(describeSession).toHaveBeenCalledWith({ sessionKey: "global", agentId: "work" });
