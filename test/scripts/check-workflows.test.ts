...
describe("check-workflows", () => {
  ...
  it("lets a slow healthy Go bootstrap exceed the linter budget", () => {
    ...
    mkdirSync(binDir);
    // PATH is restricted to binDir, which has no sleep binary; delay through the
    // absolute Node executable so the bootstrap really waits 1,200ms — longer
    // than the shortened linter budget (500ms), shorter than the bootstrap one
    // (2,000ms) — and the test fails if Go is given the wrong budget.
    ...
    writeFileSync(
      ...
      [
        "#!/bin/sh",
        'if [ "$1" = "version" ]; then exit 0; fi',
        `if [ "$1" = "run" ]; then ${nodeLiteral} -e "setTimeout(function () { process.exit(0); }, 1200)"; exit 0; fi`,
        "exit 1",
        "",
      ].join("\n"),
      { mode: 0o755 },
