...
describe("prepared npm bundle", () => {
  ...
  it("prepares the root shrinkwrap from the packed AI package before sealing", () => {
    ...
    const runPack = vi.fn((directory: string, destination: string) => {
      ...
      const tarballName =
        manifest.name === "@openclaw/ai" ? `openclaw-ai-${version}.tgz` : `openclaw-${version}.tgz`;
      return execFileSync("tar", [
        "-czf",
    ...
    expect(prepareRootShrinkwrap).toHaveBeenCalledOnce();
