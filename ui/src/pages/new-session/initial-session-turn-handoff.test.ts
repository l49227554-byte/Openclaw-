...
describe("retained launcher rejection", () => {
  it.each([false, true])(
    "gives rejected destination images independent custody (volatile=%s)",
    async (volatile) => {
      const { context } = createDraftFixture();
      const client = context.gateway.snapshot.client!;
      ...
      const retain = vi.spyOn(rejected, "retainRejectedInitialTurn").mockReturnValue(volatile);
      const clearDraft = vi.fn(async () => {});
      ...
      onTestFinished(() => releaseChatAttachmentPayloads(destination));
      ...
      releaseChatAttachmentPayloads(destination);
